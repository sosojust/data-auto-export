...
const fetchTask = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      // 处理导出方式：将字符串转换为数组
      if (task.export_methods && typeof task.export_methods === 'string') {
        task.export_methods = task.export_methods.split(',').map(method => method.trim())
      } else if (!task.export_methods) {
        task.export_methods = []
      }
...
const submitForm = async () => {
  ...
  try {
    ...
    // 准备提交数据：将导出方式数组转换为字符串
    const submitData = { ...form }
    if (Array.isArray(submitData.export_methods)) {
      submitData.export_methods = submitData.export_methods.join(',')
    }
    
    const response = await api.put(`/api/tasks/${route.params.id}`, submitData)
    if (response.data.success) {
